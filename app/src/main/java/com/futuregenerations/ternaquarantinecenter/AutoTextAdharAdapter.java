...
public class AutoTextAdharAdapter extends ArrayAdapter<Patients> {

    private Context context;
    private List<Patients> patientsDataList;
    List<Patients> suggestions;

    public AutoTextAdharAdapter(@NonNull Context context, @NonNull List<Patients> dataList) {
    ...
    private Filter filter = new Filter() {
        ...
        protected FilterResults performFiltering(CharSequence constraint) {
            ...
            if (constraint == null || constraint.length() == 0) {
                ...
                for (Patients data : patientsDataList) {
                    if (data.getAdhar().toLowerCase().contains(filterPattern)) {
        ...
        public CharSequence convertResultToString(Object resultValue) {
            return ((Patients) resultValue).getAdhar();
        }
    };
}
